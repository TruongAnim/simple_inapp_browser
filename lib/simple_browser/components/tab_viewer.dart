...
class ScrollableTab extends StatefulWidget {
  ...
  const ScrollableTab({Key? key, required this.child, this.top = 0.0, this.onTap}) : super(key: key);
...
class TabViewer extends StatefulWidget {
  ...
  const TabViewer({Key? key, required this.children, this.onTap, this.currentIndex = 0}) : super(key: key);
...
class _TabViewerState extends State<TabViewer> with SingleTickerProviderStateMixin {
      ...
      body: GestureDetector(
          ...
          onVerticalDragEnd: (details) {
            ...
            _timer = Timer.periodic(const Duration(milliseconds: 16), (timer) {
              if (positions.isEmpty || (deceleration < 0 && dy <= 0) || (deceleration >= 0 && dy >= 0)) {
                _timer?.cancel();
                return;
              }
              ...
              if (index == focusedIndex && index != 0 && index != positions.length - 1) {
                opacity = 0.15;
                  ...
                  (index < positions.length - 3 &&
                      positions[index] >= MediaQuery.of(context).size.height - TAB_VIEWER_BOTTOM_OFFSET_3)) {
                opacity = 0.0;
              }
  ...
  void updatePositions(double dy) {
    positions[focusedIndex] = focusedIndex != 0 ? positions[focusedIndex] + dy : 0.0;
    ...
    if (focusedIndex == 0 && dy < 0 && positions.isNotEmpty && focusedIndex + 1 < positions.length) {
      focusedIndex++;
